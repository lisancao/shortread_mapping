...
# Files
FASTQ=$WORKDIR/$SAMPLE.$ID.fastq.gz
SAM_FILE=$WORKDIR/$SAMPLE.$ID.sam
BAM_FILE=$WORKDIR/$SAMPLE.$ID.bam
MACS2_DIR=$WORKDIR/MACS2
CONTROL=$WORKDIR/${SAMPLE}_$ID.Control.bam
PEAKS_FILE=$MACS2_DIR/${SAMPLE}_$ID.peaks.narrowPeak
BIGWIG_FILE=$WORKDIR/$ID.$SAMPLE.bw

# Download ENCODE data
wget https://www.encodeproject.org/files/ENCFF978HHN/@@download/ENCFF978HHN.fastq.gz
wget https://www.encodeproject.org/files/ENCFF762NSE/@@download/ENCFF762NSE.fastq.gz
wget https://www.encodeproject.org/files/ENCFF639VKM/@@download/ENCFF639VKM.bam
cat $WORKDIR/ENCFF978HHN.fastq.gz $WORKDIR/ENCFF762NSE.fastq.gz > $FASTQ
rm $WORKDIR/ENCFF978HHN.fastq.gz $WORKDIR/ENCFF762NSE.fastq.gz
mv $WORKDIR/ENCFF639VKM.bam $CONTROL
